# -*- coding: utf-8 -*-
"""1000_0.5s_ORIGINAL.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1MaXzBLalo9G359ihHC8h6-0OjVRiySIN
...
output_raw_root = "/content/drive/MyDrive/PREPROCESSING_0.5s/CONV/MONO_SR_sem_trunc_mod"
csv_output_dir  = "/content/drive/MyDrive/PREPROCESSING_0.5s/CONV/METRICS_SR_sem_trunc_mod"
checkpoint_file = "/content/drive/MyDrive/PREPROCESSING_0.5s/CONV/checkpoint_SR_sem_trunc_mod.txt"
...
def pad_or_trim(sig, length):
    """Garante que o sinal tenha exatamente `length` amostras."""
    length = int(length)
...
def load_audio(path):
    ...
    return pad_or_trim(audio, int(fs_target * audio_duration))

def load_rir(path):
    """Carrega a RIR integral; se não existir, devolve vetor de zeros de 1 s."""
...
output_raw_root  = "/content/drive/MyDrive/PREPROCESSING_2s/CONV/MONO_1R_sem_trunc"
csv_output_dir   = "/content/drive/MyDrive/PREPROCESSING_2s/CONV/METRICS_1R_sem_trunc"
checkpoint_file  = "/content/drive/MyDrive/PREPROCESSING_2s/CONV/checkpoint_1R_sem_trunc.txt"
...
rir_speech_root  = "/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/FILT_SOM_FASE_ZERO"
rir_noise_root   = "/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/FILT_SOM_FASE_ZERO_RUIDO"
output_raw_root  = "/content/drive/MyDrive/PREPROCESSING_2s/CONV/MONO_5R_sem_trunc_mod"
csv_output_dir   = "/content/drive/MyDrive/PREPROCESSING_2s/CONV/METRICS_5R_sem_trunc_mod"
checkpoint_file  = "/content/drive/MyDrive/PREPROCESSING_2s/CONV/checkpoint_5R_sem_trunc_mod.txt"
...
# ───────────── Configurações ─────────────
IN_ROOT  = "/content/drive/MyDrive/PREPROCESSING_0.5s/CONV/MONO_SR_sem_trunc_mod"
OUT_ROOT = "/content/drive/MyDrive/PREPROCESSING_0.5s/CONV/MONO_SR_sem_trunc_mod_ESPEC"
DIRS = {
...
# ───────────── Configurações ─────────────
IN_ROOT  = "/content/drive/MyDrive/PREPROCESSING_2s/CONV/MONO_1R_sem_trunc"
OUT_ROOT = "/content/drive/MyDrive/PREPROCESSING_2s/CONV/MONO_1R_sem_trunc_ESPEC"
DIRS = {
    t: f"{OUT_ROOT}/{t if t != 'log-Mel' else 'log_mel'}"
    for t in ["log-Mel", "STFT", "CQT"]
}
...
────────────────────────────────────────────────────────────
✓ Gera log-Mel, STFT e CQT em PNG 224×224
✓ Salva em subpastas homônimas *cottm prefixo room_XX_ no nome do arquivo*
✓ Checkpoint por sala (room_X) para retomar sem repetir trabalho
✓ Exibe espectrogramas da primeira sala processada
...
# ───────────── Configurações ─────────────
IN_ROOT  = "/content/drive/MyDrive/PREPROCESSING_3s/CONV/MONO_5R_sem_trunc_mod"
OUT_ROOT = "/content/drive/MyDrive/PREPROCESSING_3s/CONV/MONO_5R_sem_trunc_mod_ESPEC"
DIRS = {
    t: f"{OUT_ROOT}/{t if t != 'log-Mel' else 'log_mel'}"
    for t in ["log-Mel", "STFT", "CQT"]
}
...
# ───────────── 1) CONFIGURAÇÕES ───────────────────────────
INPUT_DIR  = Path("/content/drive/MyDrive/PREPROCESSING_0.5s/CONV/MONO_SR_sem_trunc_mod_ESPEC/log_mel")
OUTPUT_DIR = Path("/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel")
EXT_IN     = ".png"
EXT_OUT    = ".pt"
...
# Caminho raiz
root_dir = '/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel'
...
# ───────────── 1) CONFIGURAÇÕES ───────────────────────────
SRC_DIR  = Path("/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel")
DEST_DIR = Path("/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA")
MOVE_FILES = False   # False = copia; True = move (remove da origem)
...
# ───────────── CONFIGURAÇÕES ──────────────────────────────
SRC_DIR   = Path("/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA")
TRAIN_DIR = Path("/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TREINO_VALID")
TEST_DIR  = Path("/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TESTE")
TRAIN_RATIO = 0.8
MOVE_FILES  = True          # False = copia, True = move
...
dirs = {
    "Treino/Validação": "/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TREINO_VALID",
    "Teste"           : "/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TESTE"
}
...
───────────────────────────────────────────────────────────────────────────────
• Treina (e retoma) um ViT-B/16 multitarefa com cabeças por banda (125-4000 Hz)
  para estimar C50 e T60 em 6 bandas de oitava, usando validação K-Fold.
• Estrutura de pastas:
...
• Ajustes: One-CycleLR, gradient clipping, dropout leve nas cabeças,
  early-stopping removido.
• **NOVO**: retomada automática + descarte imediato de tensores corrompidos.
"""
...
TRAIN_DIR   = Path("/content/drive/MyDrive/PREPROCESSING_0.5s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TREINO_VALID")
LABELS_PATH = Path("/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/PARÂMETROS/merged_params.csv")
MODEL_DIR   = Path("/content/drive/MyDrive/MODELOS COMPARATIVOS_0.5/logmel_SR_mod_vit_mod")
MODEL_DIR.mkdir(parents=True, exist_ok=True)
...
class SpectroDataset(Dataset):
    """
    Se o tensor .pt estiver corrompido, o arquivo é removido
    do disco e o amostra é trocada por outra válida.
    """
    def __init__(self, df: pd.DataFrame, tensors_dir: Path):
        self.df = df.reset_index(drop=True)
        self.tensors_dir = tensors_dir
        # marca linhas já testadas como corrompidas para não re-tentar
        self._invalid = set()
    ...
    def __getitem__(self, idx: int):
        while True:
            # tenta até achar um tensor válido
            row = self.df.iloc[idx]
            if idx in self._invalid:
                idx = random.randrange(len(self.df))
                continue

            tensor_path = self.tensors_dir / row["tensor_filename"]
            try:
                x = torch.load(tensor_path)
            except (EOFError, RuntimeError, pickle.UnpicklingError) as err:
                # arquivo corrompido → apaga e evita novas tentativas
                print(f"[SpectroDataset] Arquivo corrompido removido: {tensor_path}")
                try:
                    tensor_path.unlink(missing_ok=True)
                except Exception as exc:
                    print(f"[SpectroDataset] Falha ao remover {tensor_path}: {exc}")
                self._invalid.add(idx)
                idx = random.randrange(len(self.df))
                continue  # tenta outro índice
...
class MultiHeadViT(nn.Module):
    ...
    def forward(self, x):
        feats = self.backbone(x)          # token CLS
        return {f: head(feats) for f, head in self.heads.items()}
...
def find_last_epoch(ckpt_dir: Path, fold: int) -> Tuple[int, Path | None, Path | None]:
...
for fold, (tr_idx, v_idx) in enumerate(kf.split(df), 1):
    ...
    if ckpt_path is not None:
        ...
        print(f">>> Retomando a partir da época {last_epoch} ({ckpt_path.name})")
    else:
...
• Estrutura de pastas:
    ...
    – CSV global de métricas (RMSE, MAE, R², PCC) (append-only)
• Ajustes: One-CycleLR, gradient clipping, dropout leve nas cabeças.
• **NOVO** : retomada automática + descarte imediato de tensores corrompidos ou
  ausentes (FileNotFoundError) por qualquer worker.
"""
...
# ─────────────────── Caminhos principais ────────────────────
TRAIN_DIR   = Path("/content/drive/MyDrive/PREPROCESSING_3s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TREINO_VALID")
LABELS_PATH = Path("/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/PARÂMETROS/merged_params.csv")
MODEL_DIR   = Path("/content/drive/MyDrive/MODELOS COMPARATIVOS_3/logmel_SR_mod_vit_mod")
MODEL_DIR.mkdir(parents=True, exist_ok=True)
...
# ───────────────────── Dataset ──────────────────────
class SpectroDataset(Dataset):
    """Lê tensores .pt; se corrompido ou ausente → descarta e sorteia outro."""
    def __init__(self, df: pd.DataFrame, tensors_dir: Path):
        self.df = df.reset_index(drop=True)
        self.tensors_dir = tensors_dir
        self._invalid = set()
        ...
        while True:
            ...
            try:
                x = torch.load(tensor_path)
                    ...
                    pickle.UnpicklingError, FileNotFoundError):
                print(f"[SpectroDataset] Arquivo inválido removido: {tensor_path}")
                tensor_path.unlink(missing_ok=True)
                self._invalid.add(idx)
                idx = random.randrange(len(self))
                continue

            if x.shape[0] == 1:                          # 1-canal → 3-RGB
                x = x.repeat(3, 1, 1)
            if x.shape[-2:] != (224, 224):               # garante 224×224
                x = F.interpolate(x.unsqueeze(0), size=(224, 224),
...
def compute_metrics(pred, true, sc_c50, sc_t60):
    ...
    for f in FREQUENCIES:
        ...
        out[f] = {"C50": stats(t_c, p_c),
                  "T60": stats(t_t, p_t)}
    return out

def find_last_epoch(ckpt_dir: Path, fold: int) -> Tuple[int, Path | None, Path | None]:
    pat = re.compile(rf"fold{fold}_epoch(\d+)\.pth$")
    epochs = [(int(m.group(1)), p) for p in ckpt_dir.glob(f"fold{fold}_epoch*.pth")
...
for fold, (tr_idx, v_idx) in enumerate(kf.split(df), 1):
    ...
    if ckpt_path:
        ...
        if opt_path:
            optimizer.load_state_dict(torch.load(opt_path, map_location=device))
            # move tensores do optimizer para GPU
            for st in optimizer.state.values():
                for k, v in st.items():
                    if torch.is_tensor(v):
                        st[k] = v.to(device)
        start_epoch = last_epoch + 1
        print(f">>> Retomando da época {last_epoch} ({ckpt_path.name})")
    else:
    ...
    for epoch in range(start_epoch, EPOCHS + 1):
        ...
        for x, y in tqdm(train_loader,
                         desc=f"[F{fold}] Train {epoch}", leave=False):
            x, y = x.to(device, non_blocking=True), y.to(device, non_blocking=True)
        ...
        with torch.no_grad():
            for x, y in tqdm(val_loader,
                             desc=f"[F{fold}] Val {epoch}", leave=False):
                x, y = x.to(device, non_blocking=True), y.to(device, non_blocking=True)
                outs = model(x)
                for i, f in enumerate(FREQUENCIES):
                    pred_v[f].append(outs[f].cpu())
        ...
        train_m = compute_metrics(pred_t, true_t, scalers_c50, scalers_t60)
        val_m   = compute_metrics(pred_v, true_v, scalers_c50, scalers_t60)
        val_r2_mean = np.mean([val_m[b][v][2] for b in FREQUENCIES
                                                 for v in ("C50", "T60")])
        ...
        # ───────── CSV global ─────────
        with CSV_METRICS.open("a", encoding="utf-8") as fcsv:
            for band in FREQUENCIES:
                for var in ("C50", "T60"):
                    rt, mt, r2t, pcct = train_m[band][var]
                    rv, mv, r2v, pccv = val_m[band][var]
                    fcsv.write(f"{fold},{epoch},{band},{var},"
                               f"{rt:.6f},{mt:.6f},{r2t:.6f},{pcct:.6f},"
                               f"{rv:.6f},{mv:.6f},{r2v:.6f},{pccv:.6f}\n")
...
# GRAD-CAM - EXPLANAIBLE AI
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
test_multihead_frequency_improved.py

Avalia o modelo multitarefa treinado (cabeças por frequência) no conjunto de teste.
- Carrega o backbone e cabeças salvos
- Carrega scalers de C50 e T60
- Processa tensores de teste e labels em merged_params.csv
- Imprime métricas (RMSE, MAE, R2) por cabeça e variável
- Salva CSV com previsões e valores reais invertidos
"""
import os
import warnings
import pickle
...
def main():
    ...
    # 1) Paths
    TEST_DIR     = Path("/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/ESPECTROGRAMAS_TENSORES_PARA_TESTE")
    LABELS_PATH  = Path("/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/PARÂMETROS/merged_params.csv")
    MODEL_DIR    = Path("/content/drive/MyDrive/Modelos_sem_KFOLD_ajustado_por_freq_opt")
    CKPT_DIR     = MODEL_DIR / "checkpoints"
    SCALER_DIR   = MODEL_DIR / "scalers"
    OUTPUT_CSV   = MODEL_DIR / "test_results.csv"

    # 2) Frequências e variáveis
    FREQ_BANDS = (125, 250, 500, 1000, 2000, 4000)
    FREQUENCIES = [str(b) for b in FREQ_BANDS]
    C50_VARS = [f"C50_{b}" for b in FREQ_BANDS]
    T60_VARS = [f"T60_{b}" for b in FREQ_BANDS]

    # 3) Carrega labels e filtra por arquivos de teste disponíveis
    df = pd.read_csv(LABELS_PATH)
    df['tensor_filename'] = df['name'].str.replace(r"\.wav$", "", regex=True) + '.pt'
    ...
    # 4) Carrega scalers e aplica transformação aos labels de teste
    scalers_c50 = {}
    scalers_t60 = {}
    for var in C50_VARS + T60_VARS:
        with open(SCALER_DIR / f"scaler_{var}.pkl", 'rb') as f:
            scaler = pickle.load(f)
        band = var.split('_')[1]
        if var.startswith('C50_'):
            scalers_c50[band] = scaler
            df[var] = scaler.transform(df[[var]].values)
        else:
            scalers_t60[band] = scaler
            df[var] = scaler.transform(df[[var]].values)

    # 5) Dataset & DataLoader de teste
    class TestSpectroDataset(Dataset):
        ...
        def __getitem__(self, idx):
            ...
            return x, y, row['tensor_filename']

    BATCH_SIZE = 16
    test_loader = DataLoader(
        TestSpectroDataset(df, TEST_DIR), batch_size=BATCH_SIZE,
        shuffle=False, num_workers=4, pin_memory=True
    )

    # 6) Define modelo (mesma arquitetura do treinamento)
    head_configs = {
        "125": {"hidden_dim": 128},
        "250": {"hidden_dim": 96},
        "500": {"hidden_dim": 64},
        ...
        def __init__(self, head_configs):
            ...
            self.heads = nn.ModuleDict({
                f: nn.Sequential(
                    ...
                    nn.ReLU(inplace=True),
                    nn.Dropout(0.1),  # dropout genérico
                    nn.Linear(cfg['hidden_dim'], 2)
                ) for f, cfg in head_configs.items()
            })
        def forward(self, x):
    ...
    # 7) Carrega pesos salvos
    ckpt_path = CKPT_DIR / 'latest.pth'
    assert ckpt_path.exists(), f"Checkpoint não encontrado em {ckpt_path}"
    model.load_state_dict(torch.load(ckpt_path, map_location=device))
    model.eval()

    # 8) Avaliação
    preds, trues = {f: [] for f in FREQUENCIES}, {f: [] for f in FREQUENCIES}
    filenames = []

    with torch.no_grad():
        for x, y, fnames in tqdm(test_loader, desc="Test Evaluation"):
            x = x.to(device)
            outs = model(x)
            # coleta label real (escalada) e previsões (escaladas)
            for i, f in enumerate(FREQUENCIES):
                preds[f].append(outs[f].cpu().numpy())
                trues[f].append(y[:, i, :].cpu().numpy())
            filenames.extend(fnames)

    # 9) Consolida resultados e calcula métricas
    results = { 'tensor_filename': filenames }
    metrics = {}
    for f in FREQUENCIES:
        p = np.vstack(preds[f])  # shape (N,2)
        t = np.vstack(trues[f])
        # desfaz escalonamento
        ...
        # adiciona ao results
        results[f"C50_{f}_REAL"] = t_c50
        results[f"C50_{f}_PRED"] = p_c50
        results[f"T60_{f}_REAL"] = t_t60
        results[f"T60_{f}_PRED"] = p_t60
        # calcula métricas
        metrics[f] = {
            'C50': {
                'RMSE': np.sqrt(mean_squared_error(t_c50, p_c50)),
                'MAE': mean_absolute_error(t_c50, p_c50),
                'R2': r2_score(t_c50, p_c50)
            },
            'T60': {
                'RMSE': np.sqrt(mean_squared_error(t_t60, p_t60)),
                'MAE': mean_absolute_error(t_t60, p_t60),
                'R2': r2_score(t_t60, p_t60)
            }
        }

    # 10) Imprime métricas consolidadas
    print("\n=== Test Metrics ===")
    print("--- C50 Heads ---")
    for f in FREQUENCIES:
        m = metrics[f]['C50']
        print(f"C50_{f}: RMSE={m['RMSE']:.4f}, MAE={m['MAE']:.4f}, R2={m['R2']:.4f}")
    print("--- T60 Heads ---")
    for f in FREQUENCIES:
        m = metrics[f]['T60']
        print(f"T60_{f}: RMSE={m['RMSE']:.4f}, MAE={m['MAE']:.4f}, R2={m['R2']:.4f}")

    # 11) Salva CSV de resultados
    df_out = pd.DataFrame(results)
    df_out.to_csv(OUTPUT_CSV, index=False)
    print(f"\nResultados de teste salvos em: {OUTPUT_CSV}")
...
def main():
    ...
    # 1) Paths
    TEST_DIR     = Path("/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/ESPECTROGRAMAS_TENSORES_COM_RUIDO_PARA_TESTE_mod")
    LABELS_PATH  = Path("/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/PARÂMETROS/merged_params.csv")
    MODEL_DIR    = Path("/content/drive/MyDrive/Modelos_sem_KFOLD_ajustado_por_freq_opt")
    CKPT_DIR     = MODEL_DIR / "checkpoints"
    SCALER_DIR   = MODEL_DIR / "scalers"
    OUTPUT_CSV   = MODEL_DIR / "test_results_com_ruido.csv"
...
## TREINAMENTO LOG-MEL - 1R

## TREINAMENTO LOG-MEL - 5R
"""
