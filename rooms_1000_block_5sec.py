# -*- coding: utf-8 -*-
"""1000_5s_ORIGINAL.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1fuDomUsZ91AygObRGWKoyv0HUAcoQL5L
...
csv_output_dir  = "/content/drive/MyDrive/PREPROCESSING/CONV/METRICS_SR_sem_trunc_mod"
checkpoint_file = "/content/drive/MyDrive/PREPROCESSING/CONV/checkpoint_SR_sem_trunc_mod.txt"
...
# ───────────── Configurações ─────────────
IN_ROOT  = "/content/drive/MyDrive/PREPROCESSING/CONV/MONO_SR_sem_trunc_mod"
OUT_ROOT = "/content/drive/MyDrive/PREPROCESSING/CONV/MONO_SR_sem_trunc_mod_ESPEC"
DIRS = {
...
# ───────────── 1) CONFIGURAÇÕES ───────────────────────────
INPUT_DIR  = Path("/content/drive/MyDrive/PREPROCESSING/CONV/MONO_SR_sem_trunc_mod_ESPEC/log_mel")
OUTPUT_DIR = Path("/content/drive/MyDrive/PREPROCESSING/TENSORES_DATASET/SR_mod/log_mel")
EXT_IN     = ".png"
EXT_OUT    = ".pt"
...
# Caminho raiz
root_dir = '/content/drive/MyDrive/PREPROCESSING/TENSORES_DATASET/SR_mod/log_mel'
...
# ───────────── 1) CONFIGURAÇÕES ───────────────────────────
SRC_DIR  = Path("/content/drive/MyDrive/PREPROCESSING/TENSORES_DATASET/SR_mod/log_mel")
DEST_DIR = Path("/content/drive/MyDrive/PREPROCESSING/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA")
MOVE_FILES = False   # False = copia; True = move (remove da origem)
...
# ───────────── CONFIGURAÇÕES ──────────────────────────────
SRC_DIR   = Path("/content/drive/MyDrive/PREPROCESSING/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA")
...
dirs = {
    "Treino/Validação": "/content/drive/MyDrive/PREPROCESSING/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TREINO_VALID",
    "Teste"            : "/content/drive/MyDrive/PREPROCESSING/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TESTE"
}
...
### MASCARAMENTO BINÁRIO

USANDO SpecAugment do torchaudio  https://docs.pytorch.org/audio/main/generated/torchaudio.transforms.FrequencyMasking.html
"""

#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""
train_smooth_curriculum_vit_checkpoint_det_expanding_mask.py
──────────────────────────────────────────────────────────────────────────────
ViT-B/16 multitarefa (6 cabeças, 125–4000 Hz) para estimar C50 e T60,
com currículo suave, gradient checkpointing manual, acumulação de gradiente
para batch efetivo 32 e mascaramento binário determinístico que expande a
máscara da banda até cobrir todo o espectrograma conforme α decai de 1→0.
"""
...
MODEL_DIR   = ROOT/"MODELOS_COMPARATIVOS/logmel_SR_curriculum_expandmask"
MODEL_DIR.mkdir(parents=True, exist_ok=True)
...
# ─── Build static band masks and compute expansion bounds ──────────────────
...
# Precompute for each band: lower_idx, upper_idx, max_extension
BAND_BOUNDS = {}
for f,mask224 in MASKS_224.items():
    arr = mask224.numpy()[:,0]  # bool array shape (224,)
    lower = int(np.argmax(arr))
    upper = int(arr.shape[0] - np.argmax(arr[::-1]) - 1)
    maxext = max(lower, arr.shape[0]-1-upper)
    BAND_BOUNDS[f] = (lower, upper, maxext)
...
# ─── Model with deterministic expanding mask ───────────────────────────────
class MultiHeadViT(nn.Module):
    def __init__(self, hidden_dim=128):
        super().__init__()
        vit = models.vit_b_16(weights=models.ViT_B_16_Weights.IMAGENET1K_V1)
    ...
    def forward(self, x_full, _, alpha):
        B, C, H, W = x_full.shape
        out = {}
        for f in FREQUENCIES:
            # fetch precomputed bounds
            lower, upper, maxext = BAND_BOUNDS[f]
            # how many bins to expand outside the band
            e = int((1 - alpha) * maxext)
            nl = max(0, lower - e)
            nu = min(H-1, upper + e)

            # build a 1D mask over frequency
            m1d = torch.zeros(H, device=x_full.device, dtype=torch.bool)
            m1d[nl:nu+1] = True  # include expanded region

            # expand to full image mask
            mask3 = m1d.unsqueeze(0).unsqueeze(0).expand(B, C, H, W)

            # apply deterministic mask: zero-out outside expanded region
            x_input = x_full.clone()
            x_input[~mask3] = 0.0

            feats = checkpoint(self.backbone, x_input)
...
start_epoch, best_r2, no_improve = 1, -float("inf"), 0
loss_w = np.ones(len(FREQUENCIES)*2, dtype=np.float32)
ckpts = sorted(MODEL_DIR.glob("epoch*.pth"),
...
if ckpts:
    ...
    print(f"Retomando do {last.name}, época {start_epoch}")
...
for epoch in range(start_epoch, EPOCHS+1):
    ...
    preds_v, trues_v = ({f:[] for f in FREQUENCIES},
                        {f:[] for f in FREQUENCIES})
    with torch.no_grad():
        for x_full, y in tqdm(val_loader, desc=f"[Val] E{epoch}", leave=False):
            x_full, y = x_full.to(device), y.to(device)
            outs = model(x_full, None, 0.0)
            batch_l = sum(criterion(outs[f], y[:,i,:]) for i,f in enumerate(FREQUENCIES))
    ...
    train_m, val_m = compute_metrics(preds_t, trues_t), compute_metrics(preds_v, trues_v)
    ...
    loss_w = (1/(rmses+1e-3) / (1/(rmses+1e-3)).sum()).astype(np.float32)
...
FILES = { "C50": BASE / "patch_vectors_C50.csv",
          "T60": BASE / "patch_vectors_T60.csv" }
BANDS = ["125","250","500","1000","2000","4000"]
OUT   = BASE / "cluster_heatmaps_labeled_PCA70"
OUT.mkdir(exist_ok=True, parents=True)
...
def preprocess(X):
    X = X / (X.max(1,keepdims=True)+1e-8)
    X = StandardScaler().fit_transform(X)
    return PCA(0.80).fit_transform(X)
...
BASE = Path("/content/drive/MyDrive/MODELOS COMPARATIVOS/"
            "logmel_SR_curriculum_checkpoint_OPT/gradcam_patch_vectors")
FILES = {
    "C50": BASE / "patch_vectors_C50.csv",
    "T60": BASE / "patch_vectors_T60.csv"
}
BANDS = ["125","250","500","1000","2000","4000"]
OUT   = BASE / "cluster_heatmaps_fixed_k5"
OUT.mkdir(exist_ok=True, parents=True)
...
for tag, csv in FILES.items():
    ...
    for band in tqdm(BANDS, desc=f"{tag} | bandas"):
        ...
        # k fixo
        k_opt = FIXED_K
        labels = KMeans(n_clusters=k_opt, n_init=10, random_state=42).fit_predict(Xp)
        sil = silhouette_score(Xp, labels) if k_opt > 1 else float('nan')
...
"""### ANÁLISE DE CLUSTERS POR PARÂMETRO E POR BANDA"""
