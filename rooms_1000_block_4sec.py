# -*- coding: utf-8 -*-
"""1000_4s_ORIGINAL.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1Ws7LsGE5r0VTB80hWaJl4pZaFqC9SJKs
...
output_base_root = Path("/content/drive/MyDrive/CONV_ORIGINAL_4/SR")
csv_output_dir   = Path("/content/drive/MyDrive/CONV_ORIGINAL_4/SNR_SR")

csv_output_dir.mkdir(parents=True, exist_ok=True)

# ─── Carrega lista de .flac ────────────────────────────────────────────────────
flacs = [
    os.path.join(dp, f)
    for dp, _, files in os.walk(speech_root_dir)
    for f in files if f.lower().endswith(".flac")
]
print(f"[DEBUG] {len(flacs)} arquivos .flac encontrados em {speech_root_dir}")
if not flacs:
    raise FileNotFoundError(f"Nenhum .flac encontrado em {speech_root_dir}")

# ─── Regex para nomes de RIRs ──────────────────────────────────────────────────
# Espera arquivos como room_0_FULL_src0_rec0_mic1.wav
rir_pattern = re.compile(r"(.+)_src(\d+)_rec(\d+)_mic([12])\.wav$", re.IGNORECASE)

# ─── Funções auxiliares ────────────────────────────────────────────────────────
def load_audio(path, target_fs=fs_target):
    try:
        x, fs = sf.read(path, always_2d=False)
        if fs != audio_fs:
            x = librosa.resample(x, orig_sr=fs, target_sr=audio_fs)
        x = librosa.resample(x, orig_sr=audio_fs, target_sr=target_fs)
        return x[: target_fs * audio_duration]
    except Exception as e:
        print(f"[ERROR] não pôde carregar {path}: {e}")
        return np.zeros(target_fs * audio_duration)

def load_rir(path):
    r, fs = sf.read(path)
    if fs != rir_fs:
        r = librosa.resample(r, orig_sr=fs, target_sr=rir_fs)
    return librosa.resample(r, orig_sr=rir_fs, target_sr=fs_target)

def convolve_and_trim(x, r):
    y = fftconvolve(x, r, mode="full") if len(r) > 0 else np.zeros_like(x)
    L = fs_target * audio_duration
    if len(y) > L:
        return y[:L]
    return np.pad(y, (0, L - len(y)), "constant")
...
# ─── Loop principal ────────────────────────────────────────────────────────────
for room_path in sorted(rir_speech_base_root.iterdir()):
    if not room_path.is_dir():
        continue

    room = room_path.name
    print(f"[DEBUG] Processando sala: {room}")

    # Encontra RIRs válidos no padrão esperado
    wavs = list(room_path.glob("*.wav"))
    valid = [(rir_pattern.match(w.name), w) for w in wavs]
    valid = [(m, w) for m, w in valid if m]
    print(f"[DEBUG] {len(valid)} RIRs válidos em {room}")

    if not valid:
        print(f"[WARN] nenhum RIR válido em {room}, pulando.")
        continue

    # Extrai listas de fontes e receptores
    sources   = sorted({int(m.group(2)) for m, _ in valid})
    receivers = sorted({int(m.group(3)) for m, _ in valid})

    # Prepara saída
    out_dir = output_base_root / room
    out_dir.mkdir(parents=True, exist_ok=True)
    csv_path = csv_output_dir / f"{room}_metrics.csv"
    ...
    for src in sources:
        for rcv in receivers:
            ...
            speech_file = random.choice(flacs)
            x = load_audio(speech_file)

            # Monta caminhos RIR para mic1 e mic2
            base = f"{room}_FULL_src{src}_rec{rcv}"
            rir1_path = room_path / f"{base}_mic1.wav"
            rir2_path = room_path / f"{base}_mic2.wav"

            if not rir1_path.exists() or not rir2_path.exists():
                print(f"[WARN] faltando {rir1_path.name} ou {rir2_path.name}, pulando src{src}-rec{rcv}")
                continue

            # Carrega e processa RIRs
            r1 = load_rir(str(rir1_path))
            r2 = load_rir(str(rir2_path))
            y1 = normalize(convolve_and_trim(x, r1))
            y2 = normalize(convolve_and_trim(x, r2))

            # Salva áudio convoluído
            out1 = out_dir / rir1_path.name
            out2 = out_dir / rir2_path.name
            sf.write(str(out1), y1, fs_target)
            sf.write(str(out2), y2, fs_target)

            # Adiciona uma linha no CSV para cada mic
            results.append({
                "room": room,
                "src":  src,
                "rcv":  rcv,
            ...
            # Exibe só para a primeira sala
            if primeira_sala:
                display(Audio(y1, rate=fs_target))
                display(Audio(y2, rate=fs_target))
                primeira_sala = False

    # Gera CSV com colunas: room, src, rcv, nome, flac
    if results:
        df = pd.DataFrame(results)
        df.to_csv(csv_path, index=False)
        print(f"[OK] CSV salvo em: {csv_path}")
    else:
        print(f"[WARN] sem resultados em {room}, CSV não criado.")

import os

# Diretório raiz a ser escaneado
root_dir = "/content/drive/MyDrive/ESPECTROGRAMAS_ORIGINAL/SR"
...
for dirpath, dirnames, filenames in os.walk(root_dir):
    # Filtra apenas os arquivos com extensão .wav (case-insensitive)
    wav_files = [f for f in filenames if f.lower().endswith(".png")]
    count = len(wav_files)
    if count > 0:
        counts_per_dir[dirpath] = count
        total_wav += count

# Exibe o resultado por subdiretório
print("Contagem de arquivos .wav por pasta:")
for path, cnt in sorted(counts_per_dir.items()):
    print(f"{path}: {cnt} arquivos")

# Exibe total geral
print(f"\nTotal de arquivos .wav encontrados: {total_wav}")
...
input_dir = "/content/drive/MyDrive/CONV_ORIGINAL_4/SR"
output_dir = "/content/drive/MyDrive/ESPECTROGRAMAS_ORIGINAL/SR"

# Criar diretório de saída principal se não existir
os.makedirs(output_dir, exist_ok=True)

# Função para salvar espectrograma como imagem PNG em RGB com tamanho 224x224
def save_spectrogram_as_image(signal, file_name, output_path, fs=16000):
    f, t, Sxx = spectrogram(signal, fs=fs, nperseg=256, noverlap=128)

    # Criar espectrograma com tamanho 224x224
    ...
    # Salvar sem bordas
    temp_path = os.path.join(output_path, f"{file_name}_temp.png")
    plt.savefig(temp_path, bbox_inches='tight', pad_inches=0, transparent=True)
    plt.close(fig)

    # Abrir imagem salva, redimensionar para 224x224 e salvar em RGB
    img = Image.open(temp_path).convert("RGB").resize((224, 224))
    final_path = os.path.join(output_path, f"{file_name}.png")
    img.save(final_path, format="PNG")
    os.remove(temp_path)

    print(f"Espectrograma salvo (RGB - 224x224): {final_path}")

    return f, t, Sxx

# Processar arquivos .wav no diretório de entrada e subdiretórios
first_room_processed = False
for root, dirs, files in os.walk(input_dir):
    files = [f for f in files if f.endswith(".wav")]  # Filtra apenas arquivos .wav
    if not files:
        continue  # Pula diretórios vazios

    room_name = os.path.relpath(root, input_dir)  # Mantém subdiretórios estruturados
    ...
    for file_name in sorted(files):
        ...
        file_name_clean = file_name.replace(".wav", "")
        spectrogram_path = os.path.join(room_output_dir, f"{file_name_clean}.png")

        if os.path.exists(spectrogram_path):
            print(f"Espectrograma já existe: {spectrogram_path}, pulando...")
            continue

        try:
            fs, signal = wav.read(wav_path)
        except Exception as e:
            print(f"Erro ao ler {wav_path}: {e}")
            continue

        print(f"Processando: {wav_path}")

        # Salvar espectrograma como imagem RGB 224x224
        f, t, Sxx = save_spectrogram_as_image(signal, file_name_clean, room_output_dir, fs)

        # Mostrar espectrograma APENAS da PRIMEIRA ROOM
        if not first_room_processed:
            fig, ax = plt.subplots()
            ax.pcolormesh(t, f, 10 * np.log10(Sxx + 1e-10), shading="gouraud", cmap="inferno")
            ax.set_title(f"Espectrograma - {room_name}/{file_name_clean}")
            plt.show()

    # Marcar que a primeira room já foi processada
    if not first_room_processed:
        first_room_processed = True

print(f"Processamento concluído. Espectrogramas salvos em: {output_dir}")
...
base_dataset_path = '/content/drive/MyDrive/ESPECTROGRAMAS_ORIGINAL/SR'
tensor_save_dir = '/content/drive/MyDrive/TENSORES_ORIGINAL/SR'
...
# Percorrer recursivamente todos os subdiretórios
for root, dirs, files in os.walk(base_dataset_path):
    image_files = [file for file in files if file.lower().endswith(('png', 'jpg', 'jpeg'))]

    if len(image_files) > 0:
        print(f"Processando imagens no diretório: {root}")
        room_id = os.path.basename(root)  # Nome da sala (room_0, room_1, etc.)
        ...
        for file in image_files:
            ...
            with Image.open(image_path) as img:
                ...
                combined_name = f"{room_id}_{image_name}"  # Nome combinado da sala e imagem
                image_names.append(combined_name)

                # Caminho para salvar o tensor individualmente
                save_tensor_path = os.path.join(tensor_save_dir, f"{combined_name}.pt")

                # Salvar tensor individualmente
                torch.save(image_tensor, save_tensor_path)

print(f"Tensores individuais salvos em: {tensor_save_dir}")

import os

# Caminho para o diretório
directory = '/content/drive/MyDrive/TENSORES_ORIGINAL/SR'

# Contar arquivos .pt no diretório
file_count = len([file for file in os.listdir(directory) if file.endswith('.pt')])

# Exibir o número de arquivos .pt
print(f"Quantidade de arquivos .pt no diretório: {file_count}")
...
# ───────────── CONFIGURAÇÕES ──────────────────────────────
SRC_DIR   = Path("/content/drive/MyDrive/TENSORES_ORIGINAL/SR")
TRAIN_DIR = Path("/content/drive/MyDrive/TENSORES_ORIGINAL/SR_TREINO_VALID")
TEST_DIR  = Path("/content/drive/MyDrive/TENSORES_ORIGINAL/SR_TESTE")
TRAIN_RATIO = 0.9
...
"""
rename_room_tensors.py

Percorre recursivamente os diretórios de treino e teste,
e renomeia arquivos como:
  room_999_room_999_FULL_src4_rec4_mic2.pt
para:
  room_999_FULL_src4_rec4_mic2.pt
"""

import re
from pathlib import Path

def renomear_arquivos(base_dir: Path):
    """
    Renomeia todos os arquivos .pt em base_dir (e subpastas)
    que tenham o prefixo duplicado room_NNN_room_NNN_...
    """
    padrao = re.compile(r'^(room_\d+)_\1_(.+)\.pt$')
    count = 0

    for filepath in base_dir.rglob('*.pt'):
        nome = filepath.name
        m = padrao.match(nome)
        if m:
            novo_nome = f"{m.group(1)}_{m.group(2)}.pt"
            destino = filepath.with_name(novo_nome)
            filepath.rename(destino)
            print(f"Renomeado: {nome} → {novo_nome}")
            count += 1

    print(f"\nTotal de arquivos renomeados em '{base_dir}': {count}")

if __name__ == "__main__":
    # caminhos de treino e teste
    treino = Path("/content/drive/MyDrive/TENSORES_ORIGINAL/SR_TREINO_VALID")
    teste  = Path("/content/drive/MyDrive/TENSORES_ORIGINAL/SR_TESTE")

    print("→ Processando diretório de treino …")
    renomear_arquivos(treino)

    print("\n→ Processando diretório de teste …")
    renomear_arquivos(teste)
...
dirs = {
    "Treino/Validação": "/content/drive/MyDrive/TENSORES_ORIGINAL/SR_TREINO_VALID",
    "Teste"           : "/content/drive/MyDrive/TENSORES_ORIGINAL/SR_TESTE"
}
...
MODEL_DIR    = Path("/content/drive/MyDrive/MODELOS COMPARATIVOS/STFT_PRIMEIRO_SR_DUAS_CABECAS")
...
# -*- coding: utf-8 -*-
"""
train_multihead_frequency.py

Treina um modelo ResNet-50 multitarefa com cabeças por frequência para estimar
T60 e C50 em 6 bandas (125, 250, 500, 1000, 2000, 4000 Hz), cada cabeça com seus
hiperparâmetros. Usa escalonamento dos alvos, limpeza de NaNs/inf, barras tqdm,
e impressão detalhada de métricas por cabeça e variável (treino e validação).
Suporta retomar treinamento a partir do último checkpoint salvo.
"""
...
def main():
    # ───────────── 1) PATHS & CONFIGURAÇÕES ─────────────────
    TRAIN_DIR   = Path("/content/drive/MyDrive/TENSORES_ORIGINAL/SR_TREINO_VALID")
    LABELS_PATH = Path("/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/PARÂMETROS/merged_params.csv")
    MODEL_DIR   = Path("/content/drive/MyDrive/MODELOS COMPARATIVOS/STFT_PRIMEIRO_SR_DUAS_CABECAS_mod")
    CKPT_DIR    = MODEL_DIR / "checkpoints"
    SCALER_DIR  = MODEL_DIR / "scalers"
    VALID_PKL   = MODEL_DIR / "valid_tensors.pkl"
    os.makedirs(CKPT_DIR, exist_ok=True)
    os.makedirs(SCALER_DIR, exist_ok=True)



    # Frequências e variáveis alvo
    FREQ_BANDS = (125, 250, 500, 1000, 2000, 4000)
    ...
    if VALID_PKL.exists():
        ...
        all_files = [p.name for p in TRAIN_DIR.rglob('*.pt')]
        random.seed(42)
        valid_files = random.sample(all_files, max(1, int(0.2 * len(all_files))))
        with open(VALID_PKL, 'wb') as f:
            pickle.dump(valid_files, f)
    valid_set = set(valid_files)

    # ───────────── 3) Carrega labels e tensor filenames ─────────────────
    df = pd.read_csv(LABELS_PATH)
    TENSOR_COL = 'name'
    df['tensor_filename'] = df[TENSOR_COL].str.replace(r"\.wav$", "", regex=True) + '.pt'
    existing = {p.name for p in TRAIN_DIR.rglob('*.pt')}
    ...
    df.dropna(subset=C50_VARS + T60_VARS, inplace=True)
    after = len(df)
    print(f"Removed {before-after} rows with invalid targets.")
    df.reset_index(drop=True, inplace=True)

    # Divide em treino/val
    df['is_val'] = df['tensor_filename'].apply(lambda x: x in valid_set)
    train_df = df[~df['is_val']].reset_index(drop=True)
    val_df   = df[df['is_val']].reset_index(drop=True)

    # ───────────── 4) Escalonamento targets ─────────────────
    scalers_t60 = {}
    scalers_c50 = {}
    for var in C50_VARS + T60_VARS:
        scaler = StandardScaler()
        scaler.fit(df[[var]].values)
        with open(SCALER_DIR / f"scaler_{var}.pkl", 'wb') as f:
            pickle.dump(scaler, f)
        train_df[var] = scaler.transform(train_df[[var]].values)
        val_df[var]   = scaler.transform(val_df[[var]].values)
        band = var.split('_')[1]
        if var.startswith('C50_'):
            scalers_c50[band] = scaler
        else:
            scalers_t60[band] = scaler
    ...
    class SpectroDataset(Dataset):
        def __init__(self, df, tensors_dir):
            self.df = df
        ...
        def __getitem__(self, idx):
            row = self.df.loc[idx]
            x = torch.load(str(self.tensors_dir / row['tensor_filename']))
            labels = [[row[var] for var in C50_VARS], [row[var] for var in T60_VARS]]
            y = torch.tensor(list(zip(*labels)), dtype=torch.float)
            return x, y

    BATCH_SIZE = 16
    train_loader = DataLoader(SpectroDataset(train_df, TRAIN_DIR), batch_size=BATCH_SIZE,
                              shuffle=True, num_workers=4)
    val_loader   = DataLoader(SpectroDataset(val_df,   TRAIN_DIR), batch_size=BATCH_SIZE,
                              shuffle=False, num_workers=4)

    # ───────────── 6) Modelo multi-cabeça ─────────────────
    ...
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = MultiHeadModel(HEAD_CONFIGS).to(device)
    ...
    for f, cfg in HEAD_CONFIGS.items():
        param_groups.append({'params': model.heads[f].parameters(), 'lr': cfg['lr'], 'weight_decay': 1e-4})
    optimizer = optim.Adam(param_groups)

    # ───────────── 7b) Carregar checkpoint se existir ─────────────────
    start_epoch = 1
    # detecta último epoch_N.pth salvo
    epoch_files = [p for p in CKPT_DIR.glob('epoch_*.pth')]
    if epoch_files:
        last_epoch = max(int(p.stem.split('_')[1]) for p in epoch_files)
        ckpt_path  = CKPT_DIR / f'epoch_{last_epoch}.pth'
        model.load_state_dict(torch.load(ckpt_path, map_location=device))
        optim_path = CKPT_DIR / 'optim_latest.pth'
        if optim_path.exists():
            optimizer.load_state_dict(torch.load(optim_path, map_location=device))
            print(f"==> Otimizador reloaded from {optim_path}")
        start_epoch = last_epoch + 1
        print(f"==> Resuming from epoch {last_epoch}. Next start_epoch = {start_epoch}")
    ...
    for epoch in range(start_epoch, EPOCHS+1):
        ...
        train_preds, train_trues = {f: [] for f in FREQUENCIES}, {f: [] for f in FREQUENCIES}
        for x, y in tqdm(train_loader, desc=f"Train Epoch {epoch}"):
            x, y = x.to(device), y.to(device)
            optimizer.zero_grad()
            outs = model(x)
            for i, f in enumerate(FREQUENCIES):
                train_preds[f].append(outs[f].detach().cpu().numpy())
                train_trues[f].append(y[:, i, :].cpu().numpy())
        ...
        for f in FREQUENCIES:
            p = np.vstack(train_preds[f]); t = np.vstack(train_trues[f])
            p_c50 = scalers_c50[f].inverse_transform(p[:, [0]]).flatten()
            t_c50 = scalers_c50[f].inverse_transform(t[:, [0]]).flatten()
            p_t60 = scalers_t60[f].inverse_transform(p[:, [1]]).flatten()
            t_t60 = scalers_t60[f].inverse_transform(t[:, [1]]).flatten()
            train_metrics[f] = {
                'C50': {'RMSE': np.sqrt(mean_squared_error(t_c50, p_c50)), 'MAE': mean_absolute_error(t_c50, p_c50), 'R2': r2_score(t_c50, p_c50)},
                'T60': {'RMSE': np.sqrt(mean_squared_error(t_t60, p_t60)), 'MAE': mean_absolute_error(t_t60, p_t60), 'R2': r2_score(t_t60, p_t60)},
            }

        # Validação
        model.eval()
        val_total_loss = 0.0
        val_preds, val_trues = {f: [] for f in FREQUENCIES}, {f: [] for f in FREQUENCIES}
        with torch.no_grad():
            for x, y in tqdm(val_loader, desc=f"Val Epoch   {epoch}"):
                x, y = x.to(device), y.to(device)
                outs = model(x)
                for i, f in enumerate(FREQUENCIES):
                    val_preds[f].append(outs[f].cpu().numpy())
                    val_trues[f].append(y[:, i, :].cpu().numpy())
                val_total_loss += sum(criterion(outs[f], y[:, i, :]).item() * x.size(0)
                                        for i, f in enumerate(FREQUENCIES))
        ...
        val_metrics = {}
        for f in FREQUENCIES:
            p = np.vstack(val_preds[f]); t = np.vstack(val_trues[f])
            p_c50 = scalers_c50[f].inverse_transform(p[:, [0]]).flatten()
            t_c50 = scalers_c50[f].inverse_transform(t[:, [0]]).flatten()
            p_t60 = scalers_t60[f].inverse_transform(p[:, [1]]).flatten()
            t_t60 = scalers_t60[f].inverse_transform(t[:, [1]]).flatten()
        ...
        # Salva checkpoints do modelo e otimizador
        torch.save(model.state_dict(), CKPT_DIR / f'epoch_{epoch}.pth')
        torch.save(model.state_dict(), CKPT_DIR / 'latest.pth')
        torch.save(optimizer.state_dict(), CKPT_DIR / 'optim_latest.pth')

        # Impressão de métricas
        print(f"\nEpoch {epoch}/{EPOCHS} — Train L={train_loss:.4f}, Val L={val_loss:.4f}")
        print(">>> C50 Head metrics <<<")
        for f in FREQUENCIES:
            tm = train_metrics[f]['C50']; vm = val_metrics[f]['C50']
            print(f" C50_{f:>4s}: RMSE_t={tm['RMSE']:.4f}, MAE_t={tm['MAE']:.4f}, R2_t={tm['R2']:.4f} | "
                  f"RMSE_v={vm['RMSE']:.4f}, MAE_v={vm['MAE']:.4f}, R2_v={vm['R2']:.4f}")
        print("\n>>> T60 Head metrics <<<")
        for f in FREQUENCIES:
            tm = train_metrics[f]['T60']; vm = val_metrics[f]['T60']
            print(f" T60_{f:>4s}: RMSE_t={tm['RMSE']:.4f}, MAE_t={tm['MAE']:.4f}, R2_t={tm['R2']:.4f} | "
                  f"RMSE_v={vm['RMSE']:.4f}, MAE_v={vm['MAE']:.4f}, R2_v={vm['R2']:.4f}")
        print("-"*60)
...
output_raw_root = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_SR_sem_trunc_mod"
csv_output_dir  = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/METRICS_SR_sem_trunc_mod"
checkpoint_file = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/checkpoint_SR_sem_trunc_mod.txt"
...
output_raw_root  = "/content/drive/MyDrive/PREPROCESSING_4s/convolucoes/MONO_1R_sem_trunc"
csv_output_dir   = "/content/drive/MyDrive/PREPROCESSING_4s/convolucoes/METRICS_1R_sem_trunc"
checkpoint_file  = "/content/drive/MyDrive/PREPROCESSING_4s/convolucoes/checkpoint_1R_sem_trunc.txt"
...
rir_speech_root  = "/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/FILT_SOM_FASE_ZERO"
rir_noise_root   = "/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/FILT_SOM_FASE_ZERO_RUIDO"
output_raw_root  = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_5R_sem_trunc_mod"
csv_output_dir   = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/METRICS_5R_sem_trunc_mod"
checkpoint_file  = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/checkpoint_5R_sem_trunc_mod.txt"
...
def load_rir(path):
    """Carrega a RIR completa; se não existir, devolve zeros."""
    if not os.path.exists(path):
        return np.zeros(fs_target, dtype=np.float32)
    rir, fs = sf.read(path)
    if fs != fs_target:
...
def main():
    ...
    for sala in tqdm(salas, desc="Salas"):
        ...
        for mic1 in sorted(os.listdir(in_sp)):
            ...
            src_row = sources_df[sources_df.source_index == src_i].iloc[0]
            p1, p2  = np.array(rec_row.mic_pos_1), np.array(rec_row.mic_pos_2)
            ...
            for nidx, nfile in noise_files.items():
                ...
                raw_delta_no = (np.linalg.norm(npos - p2) - np.linalg.norm(npos - p1)) / speed_of_sound * fs_target
...
# ───────────── Configurações ─────────────
IN_ROOT  = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_SR_sem_trunc_mod"
OUT_ROOT = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_SR_sem_trunc_mod_ESPEC"
DIRS = {
...
# ───────────── Configurações ─────────────
IN_ROOT  = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_1R_sem_trunc"
OUT_ROOT = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_1R_sem_trunc_ESPEC"
DIRS = {
    t: f"{OUT_ROOT}/{t if t != 'log-Mel' else 'log_mel'}"
    for t in ["log-Mel", "STFT", "CQT"]
}
...
# ───────────── Configurações ─────────────
IN_ROOT  = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_5R_sem_trunc_mod"
OUT_ROOT = "/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_5R_sem_trunc_mod_ESPEC"
DIRS = {
    t: f"{OUT_ROOT}/{t if t != 'log-Mel' else 'log_mel'}"
    for t in ["log-Mel", "STFT", "CQT"]
}
...
# ───────────── 1) CONFIGURAÇÕES ───────────────────────────
INPUT_DIR  = Path("/content/drive/MyDrive/PREPROCESSING_4s/CONV/MONO_SR_sem_trunc_mod_ESPEC/log_mel")
OUTPUT_DIR = Path("/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel")
EXT_IN     = ".png"
EXT_OUT    = ".pt"
...
# Caminho raiz
root_dir = '/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel'
...
# ───────────── 1) CONFIGURAÇÕES ───────────────────────────
SRC_DIR  = Path("/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel")
DEST_DIR = Path("/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA")
MOVE_FILES = False   # False = copia; True = move (remove da origem)
...
# ───────────── CONFIGURAÇÕES ──────────────────────────────
SRC_DIR   = Path("/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA")
TRAIN_DIR = Path("/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TREINO_VALID")
TEST_DIR  = Path("/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TESTE")
TRAIN_RATIO = 0.8
MOVE_FILES  = True          # False = copia, True = move
...
dirs = {
    "Treino/Validação": "/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TREINO_VALID",
    "Teste"            : "/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TESTE"
}
...
TRAIN_DIR   = Path("/content/drive/MyDrive/PREPROCESSING_4s/TENSORES_DATASET/SR_mod/log_mel_PASTA_UNICA_TREINO_VALID")
LABELS_PATH = Path("/content/drive/MyDrive/Dissertação - Mestrado/NOVA_ALT_3/PARÂMETROS/merged_params.csv")
MODEL_DIR   = Path("/content/drive/MyDrive/MODELOS COMPARATIVOS_4s/logmel_SR_mod_vit_mod")
MODEL_DIR.mkdir(parents=True, exist_ok=True)
...
OUT_DIR    = MODEL_DIR / "maps_GRADCAM_TOTAL_01"
...
def generate_gradcams(model, loader, idx_target, avg, prefix):
    ...
    def bwd(_,g_in,g_out):
        grads.append(g_out[0].detach())
    ...
    for x,_ in tqdm(loader, desc=f"Grad-CAM {prefix}"):
        ...
        for band in FREQS:
            ...
            cam_single = gradcam(feats[-1], grads[-1])  # (batch,14,14)
    ...
    # no lugar de torch.stack, usa torch.cat para lidar com lotes finais menores
    for band, lst in cams.items():
        cam_stack = torch.cat(lst, dim=0)      # (total_samples,14,14)
...
def main():
    ...
    # ---------- salvar CSVs de previsões ----------
    RESULTS_FOLD_CSV     = MODEL_DIR / "results_per_fold.csv"
    RESULTS_ENSEMBLE_CSV = MODEL_DIR / "results_ensemble.csv"
    ...
    for fold in range(NUM_FOLDS):
        for b in FREQS:
            ...
            results_fold[f"T60_{b}_pred_fold{fold+1}"] = preds_fold[fold][b]["T60"]
    pd.DataFrame(results_fold).to_csv(RESULTS_FOLD_CSV, index=False)
    print(f"CSV por-fold salvo em: {RESULTS_FOLD_CSV}")
    ...
    for b in FREQS:
        results_ens[f"C50_{b}_REAL"]  = df[f"C50_{b}"]
        results_ens[f"T60_{b}_REAL"]  = df[f"T60_{b}"]
        results_ens[f"C50_{b}_PRED"]  = ens[b]["C50"]
        results_ens[f"T60_{b}_PRED"]  = ens[b]["T60"]
    pd.DataFrame(results_ens).to_csv(RESULTS_ENSEMBLE_CSV, index=False)
    print(f"CSV ensemble salvo em: {RESULTS_ENSEMBLE_CSV}\n")
